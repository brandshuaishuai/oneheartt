...
function Ihrss() 
{
	...
	if ( ! empty($data) ) 
	{
		foreach ( $data as $data ) 
		{
			...
			$Ihrss_package = $Ihrss_package ."IHRSS_SLIDESRARRAY[$cnt]='<a style=\"$Ihrss_slideshowgaphtml\" title=\"$Ihrss_title\"  target=\"$Ihrss_target\" href=\"$Ihrss_link\"><img style=\"display:inline;\" alt=\"$Ihrss_title\" src=\"$Ihrss_path\" /></a>';	";
			$cnt++;
		}
		?>
...
function Ihrss_install() 
{
	...
	if($wpdb->get_var("show tables like '". WP_Ihrss_TABLE . "'") != WP_Ihrss_TABLE) 
	{
		...
		$sSql = $IsSql . " VALUES ('".WP_IHRSS_PLUGIN_URL."/images/Sing_1.jpg', '#', '_blank', 'Image 1 Title', '1', 'YES', 'GROUP1', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
		
		$sSql = $IsSql . " VALUES ('".WP_IHRSS_PLUGIN_URL."/images/Sing_2.jpg' ,'#', '_blank', 'Image 2 Title', '2', 'YES', 'GROUP1', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
		
		$sSql = $IsSql . " VALUES ('".WP_IHRSS_PLUGIN_URL."/images/Sing_3.jpg', '#', '_blank', 'Image 3 Title', '1', 'YES', 'Widget', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
		
		$sSql = $IsSql . " VALUES ('".WP_IHRSS_PLUGIN_URL."/images/Sing_4.jpg', '#', '_blank', 'Image 4 Title', '2', 'YES', 'Widget', '0000-00-00 00:00:00');";
		$wpdb->query($sSql);
	}
