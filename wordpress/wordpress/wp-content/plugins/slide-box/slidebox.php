 ...
 * Version: 1.0.12
 * Author: Zotabox
 * Author URI: https://zotabox.com/dashboard/?utm_source=wordpress.com&utm_medium=Slide Box&utm_campaign=ecommerce%20plugins&authuser=anonymous
 * License: SMB 1.0
 */
...
function zb_slb_admin_init(){
    ...
    /* Register js. */
	wp_register_script( 'zb_slb_admin_init', plugins_url('assets/js/main.js?v='.time(), __FILE__) );
	wp_enqueue_script('zb_slb_admin_init');
...
function zb_slb_show_admin_messages()
{
	...
	$message_intall = '<div id="message" class="updated fade">
		    ...
		    <a href="/wp-admin/admin.php?page=zb_slb">
	...
	if ( is_plugin_active( 'slidebox/slidebox.php') ) {
...
function zb_slb_setting(){
	$domain_action = 'https://zotabox.com';
	$token_key = wp_create_nonce('update_zb_slb_code');
	...
	if(isset($access_key) && !empty($access_key) && strlen($access_key) > 0 && $ztb_status_disconnect == $connected){
		...
		$button = '<a  target="zotabox" href="'.$domain_action.'/customer/access/PluginLogin/?customer='.$ztb_id.'&access='.$access_key.'&domain_secure_id='.$domain.'&app=slb&platform=wordpress&utm_source=wordpress.com">
			Configure your tools
		</a>';
		$form = '';
	}else{
		$form = '<form class="ztb-register-form" target="_blank" method="POST" action="'.$domain_action.'/customer/access/PluginAuth?app=slb&utm_source=wordpress.com&utm_medium=Slide Box&utm_campaign=ecommerce%20plugins&token='.$token_key.'&platform=wordpress&access='.$access_key.'" id="account-info">
					...
					<div class="form-group button-wrapper">
						<input zb-plugin="zb_slb" class="ztb-submit-button" type="submit" value="Start Using Your New Tools Now" /><br><br>
	...
	<div class="ztb-wrapper">
		<div class="ztb-logo">
			<a href="https://zotabox.com/dashboard/?utm_source=wordpress.com&utm_medium=Slide Box&utm_campaign=ecommerce%20plugins&authuser=anonymous" title="Zotabox" target="zotabox">
...
function update_zb_slb_code(){
	if(isset($_REQUEST['token']) && wp_verify_nonce($_REQUEST['token'], 'update_zb_slb_code')){
		...
		if(!isset($domain) || empty($domain)){
			$redirect = admin_url('admin.php?page=zb_slb');
        	wp_safe_redirect($redirect);
		}else{
			if(wp_verify_nonce($_REQUEST['token'], 'update_zb_slb_code')){
