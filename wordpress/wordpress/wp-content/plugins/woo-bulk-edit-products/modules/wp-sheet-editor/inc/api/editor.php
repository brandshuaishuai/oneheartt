...
if (!class_exists('WP_Sheet_Editor_Factory')) {
	...
	class WP_Sheet_Editor_Factory {
		...
		function get_editor_settings($current_provider_in_page) {
			...
			$this->texts['hint_missing_column_on_scroll'] = ( defined('VGSE_ANY_PREMIUM_ADDON') && VGSE_ANY_PREMIUM_ADDON && current_user_can('manage_options') && is_admin()) ? sprintf(__('<h3>Missing column?</h3><ol><li>Try to add it automatically. <a href="%s" class="button">Click here</a></li><li>You reached the columns limit? <a href="%s" class="button">Fix it</a></li><li>Maybe the column is hidden. <button class="button"  data-remodal-target="modal-columns-visibility">Enable it</button></li><li>We can help you <a href="%s" target="_blank" class="button">Contact us</a></li></ol><button class="button">Close this</button>', VGSE()->textname), add_query_arg(array('wpse_rescan_db_fields' => 1)), admin_url('admin.php?page=vg_sheet_editor'), VGSE()->get_support_links('contact_us', 'url', 'sheet-missing-column')) : '';
