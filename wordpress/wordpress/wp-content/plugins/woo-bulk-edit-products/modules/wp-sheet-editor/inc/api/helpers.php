...
if (!class_exists('WP_Sheet_Editor_Helpers')) {
	...
	class WP_Sheet_Editor_Helpers {
		...
		function get_columns_limit() {
			$columns_limit = (!empty(VGSE()->options['be_columns_limit'])) ? (int) VGSE()->options['be_columns_limit'] : 310;
		...
		function maybe_replace_urls_with_file_ids($ids = array(), $post_id = null) {
			...
			foreach ($ids as $id) {
				if (filter_var($id, FILTER_VALIDATE_URL)) {
					...
					if (isset($this->urls_to_file_ids_cache[$id]) && empty($_REQUEST['wpse_no_cache'])) {
						if ($this->urls_to_file_ids_cache[$id]) {
							$out[] = $this->urls_to_file_ids_cache[$id];
						}
						continue;
					}
					...
					$this->urls_to_file_ids_cache[$id] = (int) $media_file_id;
				} else {
					$out[] = (int) $id;
				}
			}
