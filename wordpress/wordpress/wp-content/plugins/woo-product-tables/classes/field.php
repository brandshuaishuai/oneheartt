...
class fieldWtbp {
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlWtbp', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterWtbp::_($this, $this->adapt['html'], fieldAdapterWtbp::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameWtbp::_()->getModule('optionsWtbp')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', WTBP_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsWtbp'] = $options;
				}
			}
			$htmlContent = htmlWtbp::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlWtbp::nameToClassId($this->name). '"></div>';
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameWtbp::_()->getModule('optionsWtbp')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsWtbp'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlWtbp', $method)) {
			$output .= htmlWtbp::$method($name, $params);
			$output .= htmlWtbp::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', WTBP_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', WTBP_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', WTBP_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterWtbp::_($id,'getExtraFieldOptions',fieldAdapterWtbp::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', WTBP_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', WTBP_LANG_CODE).':<br />';
					$output .= htmlWtbp::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', WTBP_LANG_CODE).':<br />';
					$output .= htmlWtbp::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterWtbp::_($this, $this->adapt['htmlParams'], fieldAdapterWtbp::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', WTBP_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', WTBP_LANG_CODE).':<br />';
				$output .= fieldAdapterWtbp::_($params,'getFieldAttributes',  fieldAdapterWtbp::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameWtbp::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameWtbp::_()->getModule($module)->isExternal())
		   $config_xml = frameWtbp::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = WTBP_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', WTBP_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameWtbp::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsWtbp'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlWtbp::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
