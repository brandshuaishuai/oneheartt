...
class fieldAdapterWtbp {
    ...
    static public function taxDataHtml($field) {
        ...
        return '<div>'. __('Apply To', WTBP_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', WTBP_LANG_CODE). '</b><br />
                '. __('Destination', WTBP_LANG_CODE). ':'. htmlWtbp::checkboxlist('params[dest]', array('optionsWtbp' => $listOfDest)). '<br />
                '. __('Country', WTBP_LANG_CODE). ':'. htmlWtbp::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', WTBP_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', WTBP_LANG_CODE) : htmlWtbp::checkboxlist('params[categories][]', array('optionsWtbp' => $cOptions))). '<br />
                    <b>'. __('Brands', WTBP_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', WTBP_LANG_CODE) : htmlWtbp::checkboxlist('params[brands][]', array('optionsWtbp' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', WTBP_LANG_CODE).': '. htmlWtbp::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', WTBP_LANG_CODE).': '. htmlWtbp::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', WTBP_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                ...
                $output .= __('class', WTBP_LANG_CODE).':<br />';
                $output .= htmlWtbp::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', WTBP_LANG_CODE).':<br />';
    ...
    static function productFieldCategories($field){
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsWtbp'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', WTBP_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsWtbp'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
