...
class frameWtbp {
	...
	private function _checkPromoModName($activeModules) {
		foreach($activeModules as $i => $m) {
			if($m['code'] == 'supsystic_promo') {	// Well, rename it ;)
				...
				dbWtbp::query("UPDATE `@__modules` SET code = 'promo', label = 'promo' WHERE code = 'supsystic_promo'");
			}
		}
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(is_dir($moduleLocationDir. $code)) {
                    ...
                    if((bool)$m['active']) {
                        importClassWtbp($code. strFirstUp(WTBP_CODE), $moduleLocationDir. $code. DS. 'mod.php');
	...
	public function connectLang() {
		global $langOK;
		$langOK = load_plugin_textdomain(WTBP_LANG_CODE, false, WTBP_PLUG_NAME. '/languages/');		
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', WTBP_LANG_CODE));
        }
    }
    /**
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[WTBP_METHODS])
                    ...
                    if(array_key_exists($action, $permissions[WTBP_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameWtbp::_()->getModule('user')->getCurrentUserPosition();
	...
	public function isAdminPlugOptsPage() {
		...
		if(is_admin() && strpos($page, frameWtbp::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
