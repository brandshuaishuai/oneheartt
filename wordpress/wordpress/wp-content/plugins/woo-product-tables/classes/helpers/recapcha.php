...
class recapchaWtbp {
	...
	public function __construct() {
		if(!function_exists('recaptcha_get_html')) {	// In case if this lib was already included by another plugin
			importWtbp(WTBP_HELPERS_DIR. 'recaptchalib.php');
		}
	}
	static public function getInstance() {
		static $instance = NULL;
		if(empty($instance)) {
			$instance = new recapchaWtbp();
		}
		return $instance;
	}
	...
	public function getHtml() {
		if(reqWtbp::getVar('reqType') == 'ajax') {
	...
	public function check() {
		$resp = recaptcha_check_answer($this->_privateKey, 
					$_SERVER['REMOTE_ADDR'], 
					reqWtbp::getVar('recaptcha_challenge_field'),
					reqWtbp::getVar('recaptcha_response_field'));
