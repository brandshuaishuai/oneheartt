...
class htmlWtbp {
    ...
    static public function datepicker($name, $params = array('attrs' => '', 'value' => '')) {
		...
		$params = array_merge(array(
			'dateFormat' => WTBP_DATE_FORMAT_JS,
			'changeYear' => true,
    ...
    static public function img($src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '')) {
        if($usePlugPath) $src = WTBP_IMG_PATH. $src;
        return '<img src="'.$src.'" '
    ...
    static public function selectbox($name, $params = array('attrs' => '', 'options' => array(), 'value' => '')) {
        ...
        $valueLabel = isset($params['values']) && $params['values'] == 'labels';
        $existValue = isset($params['value']);
        if(!empty($params['options'])) {
            foreach($params['options'] as $k => $v) {
                $value = $valueLabel ? $v : $k;
                $selected = ($existValue && $value == $params['value'] ? 'selected="true"' : '');
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
		frameWtbp::_()->addScript('ajaxupload', WTBP_JS_PATH. 'ajaxupload.js');
        $out = '';
        if(strpos($params['url'], 'pl='. WTBP_CODE) === false)
			$params['url'] = uriWtbp::_(array('baseUrl' => $params['url'], 'pl' => WTBP_CODE));
        ...
        $out .= '<script type="text/javascript">// <!--
                jQuery(document).ready(function(){
                    new AjaxUpload("#toeUploadbut_'.$name.'", { 
                        ...
                        (empty($params['autoWtbpmit']) ? '' : ',  autoWtbpmit: "'. $params['autoWtbpmit']. '"').
                        (empty($params['responseType']) ? '' : ',  responseType: "'. $params['responseType']. '"').
                        (empty($params['onChange']) ? '' : ',  onChange: '. $params['onChange']. '').
                        (empty($params['onWtbpmit']) ? '' : ',  onWtbpmit: '. $params['onWtbpmit']. '').
    ...
    static public function statesInput($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '')) {
        ...
        $states = fieldAdapterWtbp::getStates($params['notSelected']);
        ...
        if(empty($params['doNotAddJs'])) {
            $res .= '<script type="text/javascript">
                // <!--
                if(!toeStates.length)
                    toeStates = '. utilsWtbp::jsonEncode($states). ';
    ...
    static public function countryList($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true)) {
        $params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
        $params['options'] = fieldAdapterWtbp::getCountries($params['notSelected']);
    ...
    static public function textFieldsDynamicTable($name, $params = array('value' => '', 'attrs' => '', 'options' => array())) {
        ...
        if(!empty($params['options'])) {
            ...
            foreach($params['options'] as $key => $p) {
                $pattern[$key] = htmlWtbp::text($name. '[]['. $key. ']');
            }
            ...
            elseif(!is_array($params['value'])) {
                $params['value'] = utilsWtbp::jsonDecode($params['value']);
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= __($p['label']). htmlWtbp::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
	...
	static public function slideInput($name, $params = array('attrs' => '', 'checked' => false, 'id' => '')) {
		...
		return '<a class="toeSlideShellWtbp" href="#"'. $params['attrs']. '>
			<span class="toeSlideButtWtbp"></span>
			<span class="toeSlideOnWtbp">'. __('ON'). '</span>
			<span class="toeSlideOffWtbp">'. __('OFF'). '</span>
	...
	static public function galleryBtn($name, $params = array()) {
		...
		$buttonParams = $params;
		$buttonParams['value'] = isset($params['btnVal']) ? $params['btnVal'] : sprintf(__('Select %s', WTBP_LANG_CODE), strFirstUp($galleryType));
    ...
    static public function selectFileBtn($name, $params = array()) {
        $type = isset($params['type']) ? $params['type'] : 'all';
        $params['value'] = isset($params['value']) ? $params['value'] : '';
        $buttonParams = array(
            'value' => isset($params['btnVal']) ? $params['btnVal'] : sprintf(__('Select %s', WTBP_LANG_CODE), strFirstUp($type)),
            'attrs' => 'class="wtbpSelectFileButton" data-type="'.$type.'"');
        $html = '<div class="wtbpSelectFile">'.
            self::hidden($name, array('value' => $params['value'], 'attrs' => $params['value_attrs'])).
            ($params['type'] == 'image' ? '<img src="" data-src="'.WTBP_IMG_PATH.'default.png">' : '').
            self::buttonA($buttonParams).'</div>';
        return $html;
    }
}
