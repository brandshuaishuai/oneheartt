...
abstract class tableWtbp {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(WTBP_CODE);
    ...
    public function store($data, $method = 'INSERT', $where = '') {
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothing to update', WTBP_LANG_CODE));
            return false;
        }
        ...
        if(dbWtbp::query($query)) {
			...
			$this->_addError(WTBP_TEST_MODE ? dbWtbp::getError() : __('Database error. Please contact your developer.', WTBP_LANG_CODE));
        return false;
    }
                ...
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if($validate) {
                        if (isset($this->_fields[$k]) && is_object($this->_fields[$k])) {
                            ...
                            if($errors = validatorWtbp::_($objForValidation)) {
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(WTBP_DATE_FORMAT, dbWtbp::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
