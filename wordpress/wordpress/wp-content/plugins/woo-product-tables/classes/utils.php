...
class utilsWtbp {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsWtbp::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsWtbp::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameWtbp::_()->getModule($m)) 
                return false;
            if(!is_dir(frameWtbp::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
