    ...
    function importClassWtbp($class, $path = '') {
        if(!class_exists($class)) {
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, WTBP_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], WTBP_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
...
if(!function_exists('toeGetClassNameWtbp')) {
    function toeGetClassNameWtbp($class) {
        $className = '';
		if(class_exists($class. strFirstUp(WTBP_CODE))) 
			$className = $class. strFirstUp(WTBP_CODE);
