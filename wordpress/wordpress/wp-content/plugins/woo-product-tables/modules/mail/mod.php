...
class mailWtbp extends moduleWtbp {
	...
	public function init() {
		parent::init();
		//dispatcherWtbp::addFilter('optionsDefine', array($this, 'addOptions'));
	}
	...
	private function _getSmtpMailer() {
		if(!$this->_smtpMailer) {
			...
			$smtpSecure = frameWtbp::_()->getModule('options')->get('smtp_secure');
			if(!empty($smtpSecure)) {
				$this->_smtpMailer->SMTPSecure = $smtpSecure; // secure transfer enabled REQUIRED for GMail
			}
			$this->_smtpMailer->Host = trim(frameWtbp::_()->getModule('options')->get('smtp_host'));
			$this->_smtpMailer->Port = trim(frameWtbp::_()->getModule('options')->get('smtp_port')); 
			$this->_smtpMailer->Username = trim(frameWtbp::_()->getModule('options')->get('smtp_login'));  
			$this->_smtpMailer->Password = trim(frameWtbp::_()->getModule('options')->get('smtp_pass'));
		}
	...
	private function _getSendMailMailer() {
		if(!$this->_sendMailMailer) {
			...
			$sendMailPath = trim(frameWtbp::_()->getModule('options')->get('sendmail_path'));
		...
		switch($type) {
			...
			case 'wp_mail': default:
				...
				if(empty($ts_mail_errors)) {
					$ts_mail_errors[] = __('Cannot send email - problem with send server', WTBP_LANG_CODE);
				}
	...
	public function addOptions($opts) {
		$opts[ $this->getCode() ] = array(
			'label' => __('Mail', WTBP_LANG_CODE),
			'opts' => array(
				'mail_function_work' => array('label' => __('Mail function tested and work', WTBP_LANG_CODE), 'desc' => ''),
				'notify_email' => array('label' => __('Notify Email', WTBP_LANG_CODE), 'desc' => __('Email address used for all email notifications from plugin', WTBP_LANG_CODE), 'html' => 'text', 'def' => get_option('admin_email')),
			),
		);
		return $opts;
	}
