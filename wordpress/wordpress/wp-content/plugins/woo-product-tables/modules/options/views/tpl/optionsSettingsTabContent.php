...
<section>
	<form id="wtbpSettingsForm" class="wtbpInputsWithDescrForm">
						...
						<?php if(isset($optCatData['opts']) && !empty($optCatData['opts'])) { ?>
							<?php foreach($optCatData['opts'] as $optKey => $opt) { ?>
								...
								>
										...
										<?php if(isset($opt['pro']) && !empty($opt['pro'])) { ?>
											<span class="wtbpProOptMiniLabel">
												<a href="<?php echo $opt['pro']?>" target="_blank">
													<?php _e('PRO option', WTBP_LANG_CODE)?>
												</a>
											</span>
										<?php }?>
									</th>
									...
									<td class="col-w-1perc">
										<?php echo htmlWtbp::$htmlType('opt_values['. $optKey. ']', $htmlOpts)?>
									</td>
									<td class="col-w-60perc">
										<div id="wtbpFormOptDetails_<?php echo $optKey?>" class="wtbpOptDetailsShell">
										<?php switch($optKey) {
		...
		<?php echo htmlWtbp::hidden('mod', array('value' => 'options'))?>
		<?php echo htmlWtbp::hidden('action', array('value' => 'saveGroup'))?>
	</form>
	<br />
</section>