...
class promoControllerWtbp extends controllerWtbp {
    public function welcomePageSaveInfo() {
		...
		if($this->getModel()->welcomePageSaveInfo(reqWtbp::get('get'))) {
			$res->addMessage(__('Information was saved. Thank you!', WTBP_LANG_CODE));
		} else {
			...
			if($validate) {
				...
				foreach($validate as $v) {
					...
					switch($v) {
						case 'notEmpty':
							if(empty($data[ $fName ])) {
								$error = $fData['html'] == 'selectbox' ? __('Please select %s', WTBP_LANG_CODE) : __('Please enter %s', WTBP_LANG_CODE);
								$error = sprintf($error, $fData['label']);
							}
							break;
						case 'email':
							if(!is_email($data[ $fName ])) 
								$error = __('Please enter valid email address', WTBP_LANG_CODE);
							break;
					}
					if(!empty($error)) {
	...
	public function saveDeactivateData() {
		...
		if($this->getModel()->saveDeactivateData(reqWtbp::get('post'))) {
			$res->addMessage(__('Thank you for Feedback!', WTBP_LANG_CODE));
		} else
			$res->pushError($this->getModel()->getErrors());
		$res->ajaxExec();
	}
