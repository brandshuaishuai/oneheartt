...
class promoWtbp extends moduleWtbp {
	...
	public function checkAdminPromoNotices() {
		...
		return;
		if(!frameWtbp::_()->isAdminPlugOptsPage())	// Our notices - only for our plugin pages for now
			return;
		$notices = array();
		// Start usage
		...
		if($startUsage) {	// Already saved
			$rateMsg = sprintf(__("<h3>Hey, I noticed you just use %s over a week – that’s awesome!</h3><p>Could you please do me a BIG favor and give it a 5-star rating on WordPress? Just to help us spread the word and boost our motivation.</p>", WTBP_LANG_CODE), WTBP_WP_PLUGIN_NAME);
			$rateMsg .= '<p><a href="https://wordpress.org/support/view/plugin-reviews/popup-by-supsystic?rate=5#postform" target="_blank" class="button button-primary" data-statistic-code="done">'. __('Ok, you deserve it', WTBP_LANG_CODE). '</a>
			<a href="#" class="button" data-statistic-code="later">'. __('Nope, maybe later', WTBP_LANG_CODE). '</a>
			<a href="#" class="button" data-statistic-code="hide">'. __('I already did', WTBP_LANG_CODE). '</a></p>';
			$enbPromoLinkMsg = sprintf(__("<h3>More then eleven days with our %s plugin - Congratulations!</h3>", WTBP_LANG_CODE), WTBP_WP_PLUGIN_NAME);
			$enbPromoLinkMsg .= __('<p>On behalf of the entire <a href="https://woobewoo.com" target="_blank">woobewoo.com</a> company I would like to thank you for been with us, and I really hope that our software helped you.</p>', WTBP_LANG_CODE);
			$enbPromoLinkMsg .= __('<p>And today, if you want, - you can help us. This is really simple - you can just add small promo link to our site under your PopUps. This is small step for you, but a big help for us! Sure, if you don\'t want - just skip this and continue enjoy our software!</p>', WTBP_LANG_CODE);
			$enbPromoLinkMsg .= '<p><a href="#" class="button button-primary" data-statistic-code="done">'. __('Ok, you deserve it', WTBP_LANG_CODE). '</a>
			<a href="#" class="button" data-statistic-code="later">'. __('Nope, maybe later', WTBP_LANG_CODE). '</a>
			<a href="#" class="button" data-statistic-code="hide">'. __('Skip', WTBP_LANG_CODE). '</a></p>';
			$enbStatsMsg = '<p>'
			               . sprintf(__('You can help us improve our plugin - by <a href="%s" data-statistic-code="hide" class="button button-primary wtbpEnbStatsAdBtn">enabling Usage Statistics</a>. We will collect only our plugin usage statistic data - to understand Your needs and make our solution better for You.', WTBP_LANG_CODE), frameWtbp::_()->getModule('options')->getTabUrl('settings'))
			               . '</p>';
			$checkOtherPlugins = '<p>'
			                     . sprintf(__('Check out <a href="%s" target="_blank" class="button button-primary" data-statistic-code="hide">our other Plugins</a>! Years of experience in WordPress plugins developers made those list unbreakable!', WTBP_LANG_CODE), frameWtbp::_()->getModule('options')->getTabUrl('featured-plugins'))
			                     . '</p>';
			$notices = array(
			...
			foreach($notices as $nKey => $n) {
				...
				if($nKey == 'enb_promo_link_msg' && (int)frameWtbp::_()->getModule('options')->get('add_love_link')) {
					unset($notices[ $nKey ]);
					continue;
				}
			}
...
//		$tabs['overview'] = array(
//			'label' => __('Overview', WTBP_LANG_CODE), 'callback' => array($this, 'getOverviewTabContent'), 'fa_icon' => 'fa-info', 'sort_order' => 5,
//		);
//		$tabs['featured-plugins'] = array(
//			'label' => __('Featured Plugins', WTBP_LANG_CODE), 'callback' => array($this, 'showFeaturedPluginsPage'), 'fa_icon' => 'fa-heart', 'sort_order' => 99,
//		);
		return $tabs;
	}
	public function addSubDestList($subDestList) {
		if(!$this->isPro()) {
			$subDestList = array_merge($subDestList, array(
				'constantcontact' => array('label' => __('Constant Contact - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'campaignmonitor' => array('label' => __('Campaign Monitor - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'verticalresponse' => array('label' => __('Vertical Response - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'sendgrid' => array('label' => __('SendGrid - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'get_response' => array('label' => __('GetResponse - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'icontact' => array('label' => __('iContact - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'activecampaign' => array('label' => __('Active Campaign - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'mailrelay' => array('label' => __('Mailrelay - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'arpreach' => array('label' => __('arpReach - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'sgautorepondeur' => array('label' => __('SG Autorepondeur - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'benchmarkemail' => array('label' => __('Benchmark - PRO', WTBP_LANG_CODE), 'require_confirm' => true),
				'infusionsoft' => array('label' => __('InfusionSoft - PRO', WTBP_LANG_CODE), 'require_confirm' => false),
				'salesforce' => array('label' => __('SalesForce - Web-to-Lead - PRO', WTBP_LANG_CODE), 'require_confirm' => false),
				'convertkit' => array('label' => __('ConvertKit - PRO', WTBP_LANG_CODE), 'require_confirm' => false),
				'myemma' => array('label' => __('Emma - PRO', WTBP_LANG_CODE), 'require_confirm' => false),
				'sendinblue' => array('label' => __('SendinBlue - PRO', WTBP_LANG_CODE), 'require_confirm' => false),
				'vision6' => array('label' => __('Vision6 - PRO', WTBP_LANG_CODE), 'require_confirm' => false),
				'vtiger' => array('label' => __('Vtiger - PRO', WTBP_LANG_CODE), 'require_confirm' => false),
				'ymlp' => array('label' => __('Your Mailing List Provider (Ymlp) - PRO', WTBP_LANG_CODE), 'require_confirm' => false),
				'fourdem' => array('label' => __('4Dem.it - PRO', WTBP_LANG_CODE), 'require_confirm' => false),
			));
		}
		return $subDestList;
	}
	...
	public function addUserExp($tabs, $popup) {
		...
		$tabs['wtbpPopupAbTesting'] = array(
			'title' => __('Testing', WTBP_LANG_CODE),
			'content' => '<a href="'. $this->generateMainLink('utm_source=plugin&utm_medium=abtesting&utm_campaign=popup'). '" target="_blank" class="button button-primary">'
	             . __('Get PRO', WTBP_LANG_CODE). '</a><br /><a href="'. $this->generateMainLink('utm_source=plugin&utm_medium=abtesting&utm_campaign=popup'). '" target="_blank">'
		...
		if(!in_array($popup['type'], array(WTBP_FB_LIKE, WTBP_IFRAME, WTBP_SIMPLE_HTML, WTBP_PDF, WTBP_AGE_VERIFY, WTBP_FULL_SCREEN))) {
			$tabs['wtbpLoginRegister'] = array(
				'title' => __('Login/Registration', WTBP_LANG_CODE),
				'content' => '<a href="'. $this->generateMainLink('utm_source=plugin&utm_medium=login_registration&utm_campaign=popup'). '" target="_blank" class="button button-primary">'
		             . __('Get PRO', WTBP_LANG_CODE). '</a><br /><a href="'. $this->generateMainLink('utm_source=plugin&utm_medium=login_registration&utm_campaign=popup'). '" target="_blank">'
	...
	public function addUserExpDesign($tabs) {
		$tabs['wtbpPopupLayeredPopup'] = array(
			'title' => __('Popup Location', WTBP_LANG_CODE),
	...
	public function checkStatisticStatus(){
		...
		if($canSend && frameWtbp::_()->getModule('user')->isAdmin()) {
			// Before this version we had many wrong data collected taht we don't need at all. Let's clear them.
			if(WTBP_VERSION == '1.3.5') {
				$clearedTrashStatData = (int) get_option(WTBP_DB_PREF. 'cleared_trash_stat_data');
				if(!$clearedTrashStatData) {
					$this->getModel()->clearUsageStat();
					update_option(WTBP_DB_PREF. 'cleared_trash_stat_data', 1);
	...
	public function getContactFormFields() {
		$fields = array(
			'name' => array('label' => __('Name', WTBP_LANG_CODE), 'valid' => 'notEmpty', 'html' => 'text'),
			'email' => array('label' => __('Email', WTBP_LANG_CODE), 'html' => 'email', 'valid' => array('notEmpty', 'email'), 'placeholder' => 'example@mail.com', 'def' => get_bloginfo('admin_email')),
			'website' => array('label' => __('Website', WTBP_LANG_CODE), 'html' => 'text', 'placeholder' => 'http://example.com', 'def' => get_bloginfo('url')),
			'subject' => array('label' => __('Subject', WTBP_LANG_CODE), 'valid' => 'notEmpty', 'html' => 'text'),
			'category' => array('label' => __('Topic', WTBP_LANG_CODE), 'valid' => 'notEmpty', 'html' => 'selectbox', 'options' => array(
				'plugins_options' => __('Plugin options', WTBP_LANG_CODE),
				'bug' => __('Report a bug', WTBP_LANG_CODE),
				'functionality_request' => __('Require a new functionality', WTBP_LANG_CODE),
				'other' => __('Other', WTBP_LANG_CODE),
			)),
			'message' => array('label' => __('Message', WTBP_LANG_CODE), 'valid' => 'notEmpty', 'html' => 'textarea', 'placeholder' => __('Hello WooBeWoo Team!', WTBP_LANG_CODE)),
		);
...
//			$this->_assetsUrl = frameWtbp::_()->getModule('comparison')->getAssetsUrl(). 'promo/';
//		}
...
//	}
	public function checkWelcome() {
		...
		if($from == 'welcome-page' && $pl == WTBP_CODE && frameWtbp::_()->getModule('user')->isAdmin()) {
	...
	public function checkProTpls($list) {
		if(!$this->isPro()) {
			//$imgsPath = frameWtbp::_()->getModule('comparison')->getAssetsUrl(). 'img/preview/';
			$imgsPath = '';
			$promoList = array(
	...
	public function loadTutorial() {
		...
		if ( is_admin() && current_user_can(frameWtbp::_()->getModule('adminmenu')->getMainCap()) ) {
