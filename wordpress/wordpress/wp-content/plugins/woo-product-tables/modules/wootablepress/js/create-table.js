(function($) {
	...
	$(document).ready(function () {
		...
		if(wtbpAddDialog.length) {
			...
			var createTable = $('#wtbpCreateTable').css('width', '100%').DataTable({
				...
				fnDrawCallback : function(){
					...
					wrapper.find('.dataTables_paginate')[0].style.display = wrapper.find('.dataTables_paginate span .paginate_button').size() > 1 ? 'block' : 'none';
			...
			$dialog = wtbpAddDialog.dialog({
				...
				create:function () {
					$(this).closest(".ui-dialog").find(".ui-dialog-buttonset button").last().addClass('wtbpDialogSave');
				}
			});
		...
		$(document.body).off('click', '.supsystic-navigation li:has(a[href$="admin.php?page=wtbp-table-press&tab=wootablepress#wtbpadd"])');
		$(document.body).on('click', '.supsystic-navigation li:has(a[href$="admin.php?page=wtbp-table-press&tab=wootablepress#wtbpadd"])', function(e){
			e.preventDefault();
			showAddDialog();
		});
...
function wtbpSetSelectAllExclude(ids, add, check, name) {
	var isAll = $('#wtbp'+name+'TableSelectAll').val() == '1',
		excludeList = $('#wtbp'+name+'TableSelectExclude').val(),
		excludeArr = excludeList.length ? excludeList.split(',') : [],
		table = $('#wtbp'+name+'Table_wrapper table');
	...
	$('#wtbp'+name+'TableSelectExclude').val(excludeArr);
}
