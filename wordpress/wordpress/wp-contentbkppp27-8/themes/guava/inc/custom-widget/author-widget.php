...
class Guava_Author_Widget extends WP_Widget
{
     public function __construct()
     {
          parent::__construct(
               'author-widget',
               __( 'CT Author', 'guava' ),
               array( 'description' => __( 'Best displayed in Home page.', 'guava' ) )
          );
      }
     ...
     public function form($instance ){
          ?>
              <p>
                 ...
                 <input type="button" class="button button-primary custom_media_button" id="custom_media_button" name="<?php echo $this->get_field_name('image_uri'); ?>" value="<?php esc_attr_e('Upload Image','guava')?>" style="margin-top:5px;" />
             </p>
             <p>
                 <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title', 'guava' ); ?></label><br />
             ...
             <p>
                 <label for="<?php echo $this->get_field_id('description'); ?>"><?php _e( 'Description', 'guava' ); ?></label><br />
...
add_action( 'widgets_init', 'guava_author_widget' ); 
function guava_author_widget(){     
    register_widget( 'Guava_Author_Widget' );

}


/*
Function to load media on widget
*/
if( !function_exists( 'guava_author_widgets_backend_enqueue' )):
function guava_author_widgets_backend_enqueue($hook){     
    wp_enqueue_script( 'guava-custom-widgets', get_template_directory_uri().'/assets/js/widgets-admin.js', array( 'jquery' ), true );
    wp_enqueue_media();
}
add_action( 'admin_enqueue_scripts', 'guava_author_widgets_backend_enqueue' );
endif;