...
class Guava_Recent_Post_Widget extends WP_Widget{
     public function __construct(){
          parent::__construct(
               'recent-post-widget',
               __( 'CT Recent Post', 'guava' ),
               array( 'description' => __( 'Best displayed in Sidebar.', 'guava' ) )
          );
     }
     ...
     public function widget( $args, $instance )
     {
       ...
       $posts_data = get_posts( $args );  
        ...
        $output .='<div class="guava-recent-posts ' . $order_by . '">';
        ...
        foreach ($posts_data as $post): setup_postdata($post);
          $output .='<div class="image">';
            ...
            if(has_post_thumbnail()):
              ...
              $output .='<a href="'.esc_url(get_permalink()).'">'.get_the_post_thumbnail( get_the_ID(), 'guava-thumb', array('class' => 'img-responsive')).'</a>';
              $output .='</figure>';
            endif;
    ...
    $instance = wp_parse_args( (array) $instance, $defaults );
          ?>
          <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php _e('Widget Title', 'guava'); ?></label>
          ...
          <p>
              <label for="<?php echo esc_attr($this->get_field_id( 'order_by' )); ?>"><?php _e('Ordered By', 'guava'); ?></label>
              <?php 
                $options = array(
                  'popular'   => __('Popular Posts', 'guava' ),
                  'recent-post'  => __('Recent Posts', 'guava'),
                  'comments'  => __('Most Commented Posts', 'guava' )
                  );
          ...
          <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'count' )); ?>"><?php _e('Count', 'guava'); ?></label>
...
add_action( 'widgets_init', 'guava_recent_post_widget' ); 
function guava_recent_post_widget(){     
    register_widget( 'Guava_Recent_Post_Widget' );
